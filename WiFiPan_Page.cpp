...
namespace WiFiPan
{
    ...
    Status Page::AddParam(const char *id, const char *label, const char *placeholder,
        ...
        for (size_t i = 0; i < used_; ++i) {
            ...
            if (std::strcmp(p.type.data(), "password") == 0) {
                html += "<div class=\"password-wrapper\">"
                        "<input type=\"password\" id=\"" + eid +
                        "\" name=\"" + eid +
                        "\" placeholder=\"" + eph +
                        "\" value=\"" + eval + "\"" + req + ">";
                html += "<span class=\"toggle-password\" onclick=\"toggleField('" + eid +
                        "')\">&#128065;</span></div>";
            } else {
                html += "<input type=\"" + etyp +
                        "\" id=\"" + eid +
                        "\" name=\"" + eid +
                        "\" placeholder=\"" + eph +
                        "\" value=\"" + eval + "\"" + req + ">";
    ...
    std::string Page::BuildConfigInject() const
    {
        ...
        for (size_t i = 0; i < used_; ++i) {
            ...
            if      (std::strcmp(p.type.data(), "password") == 0) badge_type = "pass";
            else if (std::strcmp(p.type.data(), "number")   == 0) badge_type = "int";
            else if (std::strcmp(p.type.data(), "checkbox") == 0) badge_type = "bool";
            else if (std::strcmp(p.type.data(), "select")   == 0) badge_type = "sel";
            else                                                   badge_type = "str";

            if (i > 0) out += ',';
            out += "{\"key\":\"";  JsonEscapeAppend(out, p.id.data());
            out += "\",\"type\":\""; out += badge_type;
            out += "\",\"label\":\""; JsonEscapeAppend(out, p.label.data());
            out += "\",\"default\":\""; JsonEscapeAppend(out, p.value.data());
            out += "\",\"desc\":\""; JsonEscapeAppend(out, p.placeholder.data()); /* desc reuses placeholder */
            out += "\"}";
        }
        out += "];";
        ...
        for (size_t i = 0; i < used_; ++i) {
            const Param &p = params_[i];
            if (i > 0) out += ',';
            out += '"'; JsonEscapeAppend(out, p.id.data());
            out += "\":\""; JsonEscapeAppend(out, p.value.data());
            out += '"';
        }
        out += "};</script>";
