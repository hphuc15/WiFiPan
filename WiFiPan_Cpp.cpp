...
namespace WiFiPan
{
    ...
    void Manager::HandleApEvent(esp_event_base_t base, int32_t id, void *data)
    {
        ...
        switch (id)
        {
            case WIFI_EVENT_AP_START:
            {
                ...
                ESP_LOGI(kTagAp, "AP START. SSID: %s, IP: " IPSTR ".", reinterpret_cast<char *>(ap_config_.ssid),
                         IP2STR(&ip_info.ip));
                break;
            }
    ...
    Status Manager::StartSta()
    {
        ...
        ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_STA_START, &Manager::StaEventTrampoline, this,
                                                  &priv_->sta_handle);
        if (ret != ESP_OK)
            return Status::WifiError;

        ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_STA_DISCONNECTED, &Manager::StaEventTrampoline,
                                                  this, &priv_->sta_disc_handle);
        if (ret != ESP_OK)
            return Status::WifiError;

        ret = esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &Manager::StaEventTrampoline, this,
                                                  &priv_->ip_handle);
        if (ret != ESP_OK)
            return Status::WifiError;
    ...
    Status Manager::StartAp()
    {
        ...
        ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_AP_STACONNECTED, &Manager::ApEventTrampoline,
                                                  this, &priv_->ap_connected_handle);
        if (ret != ESP_OK)
            return Status::WifiError;

        ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_AP_STADISCONNECTED,
                                                  &Manager::ApEventTrampoline, this, &priv_->ap_disconnected_handle);
        if (ret != ESP_OK)
            return Status::WifiError;

        ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_AP_START, &Manager::ApEventTrampoline, this,
                                                  &priv_->ap_start_handle);
        if (ret != ESP_OK)
            return Status::WifiError;
    ...
    Status Manager::Stop()
    {
        ...
        xEventGroupClearBits(priv_->group,
                             kEventBitApStart | kEventBitStaStart | kEventBitStaConnected | kEventBitStaDisconnected);
    ...
    Status Manager::ConfigViaAp()
    {
        ...
        EventBits_t bits =
            xEventGroupWaitBits(priv_->group, kEventBitApStart, pdFALSE, pdFALSE, pdMS_TO_TICKS(kApStartTimeoutMs));
        ...
        EventBits_t conn_bits = xEventGroupWaitBits(priv_->group, kEventBitStaConnected | kEventBitStaDisconnected,
        ...
        ESP_LOGE(kTagPortal, "Provisioned SSID '%s' did not connect", reinterpret_cast<char *>(sta_config_.ssid));
    ...
    Status Manager::AutoConnect()
    {
        ...
        EventBits_t bits = xEventGroupWaitBits(priv_->group, kEventBitStaConnected | kEventBitStaDisconnected, pdFALSE,
                                               pdFALSE, timeout);
        ...
        ESP_LOGW(kTagAuto, "Failed to connect to: %s, falling back to portal",
