...
namespace WiFiPan
{
    namespace
    {
        ...
        inline bool DnsIsPtr(uint8_t b)
        {
            return (b & 0xC0) == 0xC0;
        }
        inline uint16_t DnsFlagGetQr(uint16_t f)
        {
            return (f & kDnsFlagQrMask) >> 15;
        }
        inline uint16_t DnsFlagGetOpcode(uint16_t f)
        {
            return (f & kDnsFlagOpcodeMask) >> 11;
        }
        inline uint16_t DnsFlagSetQr(uint16_t f)
        {
            return f | kDnsFlagQrMask;
        }
        inline uint16_t DnsFlagSetAa(uint16_t f)
        {
            return f | kDnsFlagAaMask;
        }
        inline uint16_t DnsFlagSetRcode(uint16_t f, uint16_t r)
        {
            return (f & ~kDnsFlagRcodeMask) | (r & kDnsFlagRcodeMask);
        }
        ...
        const char *DnsStatusToStr(DnsStatus status)
        {
            switch (status)
            {
                case DnsStatus::Ok:
                    return "Ok";
                case DnsStatus::ErrSocket:
                    return "ErrSocket";
                case DnsStatus::ErrBind:
                    return "ErrBind";
                case DnsStatus::ErrRecv:
                    return "ErrRecv";
                case DnsStatus::ErrSend:
                    return "ErrSend";
                case DnsStatus::ErrBuild:
                    return "ErrBuild";
                case DnsStatus::ErrInvalidIp:
                    return "ErrInvalidIp";
                default:
                    return "Unknown";
            }
        ...
        DnsStatus DnsBuildResponse(const uint8_t *query_buf, size_t qname_end, uint32_t redirect_ip, uint8_t *out_buf,
                                   size_t *out_len)
        {
        ...
        DnsStatus DnsPoll(DnsServerState &srv)
        {
            ...
            ssize_t sent =
                sendto(srv.sockfd, resp, resp_len, 0, reinterpret_cast<struct sockaddr *>(&client), client_len);
            if (sent < 0)
            {
        ...
        std::string BuildScanInjectScript(uint16_t max_count)
        {
            ...
            for (uint16_t i = 0; i < fetched; i++)
            {
                ...
                if (i > 0)
                    out += ',';
                out += "{\"ssid\":\"" + ssid_json + "\",\"rssi\":" + std::to_string(ap_buf[i].rssi) +
                       ",\"enc\":" + (enc ? "true" : "false") + "}";
            }
            out += "];";
        ...
        esp_err_t HandlerWifiPost(httpd_req_t *req)
        {
            ...
            if (RecvBody(req, body, sizeof(body)) != ESP_OK)
                return ESP_FAIL;
        ...
        esp_err_t HandlerOtaPost(httpd_req_t *req)
        {
            ...
            ESP_LOGI(kTag, "/ota: writing to partition '%s' at 0x%lx", update_partition->label,
...
#define WP_ENDPOINT(m, u, h)                                                                                           \
    httpd_uri_t                                                                                                        \
    {                                                                                                                  \
        .uri = (u), .method = (m), .handler = (h), .user_ctx = nullptr                                                 \
    }
...
#undef WP_ENDPOINT
    ...
    void *Manager::StartDns(esp_ip4_addr_t ip)
    {
        ...
        if (DnsInit(h->dns, ip_str, kDnsPort) != DnsStatus::Ok || DnsStart(h->dns) != DnsStatus::Ok)
        {
            delete h;
            return nullptr;
        }
