...
namespace
{
    ...
    void PrintStatusTask(void *arg)
    {
        ...
        while (true)
        {
            ESP_LOGI(kTag, "status: connected=%d mode=%d ip=%s", wifi->IsConnected(), static_cast<int>(wifi->GetMode()),
        ...
        while (true)
        {
            ...
            if (pressed && (esp_timer_get_time() - press_start) / 1000 >= kHoldToConfigMs)
            {
                ...
                if (st == WiFiPan::Status::Ok)
                {
                    ...
                    ESP_LOGI(kTag, "dev_name=%s report_interval=%s", wifi->page().GetParam("dev_name"),
