...
typedef enum { LED_OFF, LED_WHITE, LED_BLUE, LED_YELLOW, LED_GREEN, LED_RED } led_color_t;
...
static void LedSet(led_color_t color)
{
    ...
    uint8_t r = 0, g = 0, b = 0;
    switch (color) {
    case LED_WHITE:  r = g = b = LED_BRIGHTNESS; break;
    case LED_BLUE:   b = LED_BRIGHTNESS; break;
    case LED_YELLOW: r = g = LED_BRIGHTNESS; break;
    case LED_GREEN:  g = LED_BRIGHTNESS; break;
    case LED_RED:    r = LED_BRIGHTNESS; break;
    case LED_OFF:
    default:
        break;
    }
...
static void PrintStatusTask(void *arg)
{
    ...
    while (true) {
        ...
        ESP_LOGI(TAG, "status: connected=%d mode=%d ip=%s",
                 WiFiPan_IsConnected(wifi), (int)WiFiPan_GetMode(wifi), ip);
    ...
    while (true) {
        ...
        if (pressed && (esp_timer_get_time() - press_start) / 1000 >= HOLD_TO_CONFIG_MS) {
            ...
            if (st == WIFIPAN_OK) {
                ...
                ESP_LOGI(TAG, "dev_name=%s report_interval=%s",
                         WiFiPan_Page_GetParam(wifi, "dev_name"),
